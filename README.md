...
\
Optionally, you may use the `COMPUTE_SHADERS` preprocessor option. Compute shaders are a more modern alternative to the previously implemented mode of generating cubemaps, although it will likely make very little difference in terms of computation times. It is possible, however, that they will be less prone to bugs in very large molecules. To enable them, enable the `COMPUTE_SHADERS` option as outlined above.
